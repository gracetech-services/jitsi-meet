 ...
 * @param {number} durationMs - Duration in milliseconds after which the room expires.
 * @returns {Function}
 */
export function createBreakoutRoom(name?: string, durationMs?: number) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        const state = getState();
        let { roomCounter } = state[FEATURE_KEY];
        const baseName = name || i18next.t('breakoutRooms.defaultName', { index: ++roomCounter });
        const expiresAt = durationMs ? Date.now() + durationMs : undefined;
        const subject = expiresAt ? `${baseName}${TIMEOUT_SEPARATOR}${expiresAt}` : baseName;
...
export function closeBreakoutRoom(roomId: string) {
    return async (dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        // Guard against duplicate calls — skip if room or participants are missing, to avoid duplicate XMPP commands
        if (!room || !Object.keys(room.participants || {}).length || !mainRoom) {
            return;
        }
...
export function renameBreakoutRoom(breakoutRoomJid: string, name = '') {
    return (_dispatch: IStore['dispatch'], getState: IStore['getState']) => {
        ...
        if (trimmedName.length !== 0) {
            // Preserve existing expiresAt when renaming
            const existing = getRoomByJid(getState, breakoutRoomJid);
            const subject = existing?.expiresAt
                ? `${trimmedName}${TIMEOUT_SEPARATOR}${existing.expiresAt}`
                : trimmedName;
