 ...
 * @param {IStateful} stateful - The redux store, the redux
 * {@code getState} function, or the redux state itself.
 * @returns {number|undefined} ExpiresAt timestamp if exists, undefined otherwise.
 */
export const getCurrentRoomExpiresAt = (stateful: IStateful): number | undefined => {
    const rooms = getBreakoutRooms(stateful);
    const roomId = getCurrentRoomId(stateful);

    return roomId
        ? Object.values(rooms).find(r => r.id === roomId)?.expiresAt
        : undefined;
};
